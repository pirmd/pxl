...
handle_input(font_view_t *fv, pxl_app_t *app) {
	...
	if (pxl_app_was_triggered(app, PXL_KEYB_H) || pxl_app_is_down(app, PXL_KEYB_LEFT)) {
		font_view_next_glyph(fv, -1);
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_J) || pxl_app_is_down(app, PXL_KEYB_DOWN)) {
		font_view_next_glyph(fv, fv->cols);
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_K) || pxl_app_is_down(app, PXL_KEYB_UP)) {
		font_view_next_glyph(fv, -fv->cols);
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_L) || pxl_app_is_down(app, PXL_KEYB_RIGHT)) {
		font_view_next_glyph(fv, 1);
	}
}
