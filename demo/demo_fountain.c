...
handle_fountain_input(pxl_app_t *app, particle_type_t current_type, fountain_input_t *input) {
	...
	*input = (fountain_input_t){0};
	...
	if (pxl_app_was_triggered(app, PXL_KEYB_1)) {
		input->emit_type = PARTICLE_FIRE;
		input->emit_type_changed = true;
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_2)) {
		input->emit_type = PARTICLE_SMOKE;
		input->emit_type_changed = true;
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_3)) {
		input->emit_type = PARTICLE_SPARK;
		input->emit_type_changed = true;
	}

	/* Emission rate */
	if (pxl_app_was_triggered(app, PXL_KEYB_UP) || pxl_app_was_triggered(app, PXL_KEYB_K)) {
		input->emit_rate_change = +5;
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_DOWN) || pxl_app_was_triggered(app, PXL_KEYB_J)) {
		input->emit_rate_change = -5;
	}

	/* Change particle type with keyboard or left mouse button */
	if (pxl_app_was_triggered(app, PXL_KEYB_LEFT)) {
		input->emit_type = (current_type - 1 + PARTICLE_COUNT) % PARTICLE_COUNT;
		input->emit_type_changed = true;
	}
	if (pxl_app_was_triggered(app, PXL_KEYB_RIGHT)) {
		input->emit_type = (current_type + 1) % PARTICLE_COUNT;
		input->emit_type_changed = true;
	}
	if (pxl_app_was_triggered(app, PXL_MOUSE_LEFT)) {
		input->emit_type = (current_type + 1) % PARTICLE_COUNT;
		input->emit_type_changed = true;
	}
...
handle_input(pxl_app_t *app, ui_t *ui, particle_type_t current_type, fountain_input_t *fountain_input) {
	...
	if (pxl_app_was_triggered(app, PXL_MOUSE_LEFT) || pxl_app_was_triggered(app, PXL_MOUSE_RIGHT)) {
		ui->show_pause = false;
	}
