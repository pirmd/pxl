...
/* Helpers ----------------------------------------------------------------- */
static inline bool
is_inside_scissor(int x, int y, const canvas_t *cnv) {
	return in_rect(x, y, cnv->scissor);
}
...
static inline bool is_drawn_on_line(int x, int y, int x0, int y0, int x1, int y1, rect_t scissor);
...
static inline bool
is_on_triangle(int x, int y, int x0, int y0, int x1, int y1, int x2, int y2, rect_t scissor) {
 ...
 * to ensure 100% consistency between drawing and testing. */
static inline bool
is_drawn_on_rect(int x, int y, int rx, int ry, int rw, int rh, rect_t scissor) {
	if (rw <= 0 || rh <= 0) {
		return false;
	}
	...
	/* Simulate clip_rect */
	rect_t r;
	if (!clip_rect((rect_t){rx, ry, rw, rh}, scissor, &r)) {
		return false;
	}
 ...
 * to ensure 100% consistency between drawing and testing. */
static inline bool
is_drawn_inside_fill_rect(int x, int y, int rx, int ry, int rw, int rh, rect_t scissor) {
	if (rw <= 0 || rh <= 0) {
		return false;
	}
 ...
 * to ensure 100% consistency between drawing and testing. */
static inline bool
is_drawn_on_line(int x, int y, int x0, int y0, int x1, int y1, rect_t scissor) {
	...
	if (dx >= dy) {  /* X-major line */
		...
		for (;;) {
			...
			if (y0 >= scissor.y && y0 < scissor.y + scissor.h) {
				span_t span;
				if (clip_span((span_t){x0, 1}, (span_t){scissor.x, scissor.w}, &span)) {
					if (x == span.x && y == y0) {
						return true;
					}
				}
...
/* Test Line -------------------------------------------------------------- */
static void
test_draw2d_line_horizontal(void) {
	int w = 20, h = 20;
	fixture_t f;
	...
	pix_t color = COLOR_RED;
	canvas_set_color(&f.cnv, color);

	/* Horizontal line at y=10, x=5 to 14 */
	int x0 = 5, y0 = 10, x1 = 14, y1 = 10;
	draw2d_line(&f.cnv, x0, y0, x1, y1);
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			...
			pix_t want = should_be_colored ? color : 0x00;
...
static void
test_draw2d_line_vertical(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_line_diagonal(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_line_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_rect_zero_size(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	draw2d_rect(&f.cnv, 5, 5, 0, 0);
	draw2d_rect(&f.cnv, 5, 5, 10, 0);
	draw2d_rect(&f.cnv, rx, ry, rw, rh);
...
static void
test_draw2d_rect_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_rect_clip_left_no_false_border(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	canvas_set_scissor(&f.cnv, 5, 0, 15, 20);
...
static void
test_draw2d_rect_clip_right_no_false_border(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	canvas_set_scissor(&f.cnv, 0, 0, 15, 20);
...
static void
test_draw2d_rect_clip_both_sides(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	canvas_set_scissor(&f.cnv, 5, 0, 10, 20);
...
static void
test_draw2d_rect_clip_top_no_false_border(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	canvas_set_scissor(&f.cnv, 0, 5, 20, 15);
...
static void
test_draw2d_rect_clip_bottom_no_false_border(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	canvas_set_scissor(&f.cnv, 0, 0, 20, 15);
...
static void
test_draw2d_fill_rect_basic(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	int rx = 5, ry = 5, rw = 10, rh = 10;
	draw2d_fill_rect(&f.cnv, rx, ry, rw, rh);

	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			pix_t got = *pixbuf_ptr(&f.pb, x, y);
			bool in_rect = is_drawn_inside_fill_rect(x, y, rx, ry, rw, rh, f.cnv.scissor);
			bool in_s = is_inside_scissor(x, y, &f.cnv);
...
static void
test_draw2d_fill_rect_zero_size(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	/* Note: only the last call matters for the test since all draw to the same pixbuf */
	int rx = 5, ry = 5, rw = 0, rh = 10;
	draw2d_fill_rect(&f.cnv, 5, 5, 0, 0);
	draw2d_fill_rect(&f.cnv, 5, 5, 10, 0);
	draw2d_fill_rect(&f.cnv, rx, ry, rw, rh);
...
static void
test_draw2d_fill_rect_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_fill_rect_fast_path(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_circle_zero_radius(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_circle_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_fill_circle_basic(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	int cx = 10, cy = 10, r = 5;
	draw2d_fill_circle(&f.cnv, cx, cy, r);
...
static void
test_draw2d_fill_circle_zero_radius(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	/* Note: only the last call matters for the test since all draw to the same pixbuf */
	int cx = 10, cy = 10, r = -1;
	draw2d_fill_circle(&f.cnv, cx, cy, 0);
	draw2d_fill_circle(&f.cnv, cx, cy, r);
...
static void
test_draw2d_fill_circle_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_triangle_basic(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	draw2d_triangle(&f.cnv, x0, y0, x1, y1, x2, y2);
...
static void
test_draw2d_triangle_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
static void
test_draw2d_fill_triangle_basic(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
	...
	int x0 = 5, y0 = 5, x1 = 15, y1 = 5, x2 = 10, y2 = 15;
	draw2d_fill_triangle(&f.cnv, x0, y0, x1, y1, x2, y2);
...
static void
test_draw2d_fill_triangle_outside_scissor(void) {
	int w = 20, h = 20;
	fixture_t f;
	if (!fixture_init(&ST_HERE, &f, w, h)) {
		return;
...
main(int argc, char *argv[]) {
	...
	return ST_RUN(
		/* Line tests */
		ST_T(test_draw2d_line_horizontal),
		ST_T(test_draw2d_line_vertical),
		ST_T(test_draw2d_line_diagonal),
		ST_T(test_draw2d_line_outside_scissor),

		/* Rect tests */
		ST_T(test_draw2d_rect_basic),
		ST_T(test_draw2d_rect_zero_size),
		ST_T(test_draw2d_rect_outside_scissor),
		ST_T(test_draw2d_rect_clip_left_no_false_border),
		ST_T(test_draw2d_rect_clip_right_no_false_border),
		ST_T(test_draw2d_rect_clip_both_sides),
		ST_T(test_draw2d_rect_clip_top_no_false_border),
		ST_T(test_draw2d_rect_clip_bottom_no_false_border),

		/* Fill Rect tests */
		ST_T(test_draw2d_fill_rect_basic),
		ST_T(test_draw2d_fill_rect_zero_size),
		ST_T(test_draw2d_fill_rect_outside_scissor),
		ST_T(test_draw2d_fill_rect_fast_path),

		/* Circle tests */
		ST_T(test_draw2d_circle_basic),
		ST_T(test_draw2d_circle_zero_radius),
		ST_T(test_draw2d_circle_outside_scissor),

		/* Fill Circle tests */
		ST_T(test_draw2d_fill_circle_basic),
		ST_T(test_draw2d_fill_circle_zero_radius),
		ST_T(test_draw2d_fill_circle_outside_scissor),

		/* Triangle tests */
		ST_T(test_draw2d_triangle_basic),
		ST_T(test_draw2d_triangle_outside_scissor),

		/* Fill Triangle tests */
		ST_T(test_draw2d_fill_triangle_basic),
		ST_T(test_draw2d_fill_triangle_outside_scissor)
